...
$page_title = "Customers reservation";
...
if(isset($GET['p']) && is_numeric($_GET['p'])){	
	...
	$q = "SELECT COUNT(reservation_id) FROM reservation";
...
if($_SERVER['REQUEST_METHOD']=="POST") {
	...
	if(!empty($_POST['searchs'])) {
		...
		$problem = TRUE;
		$query = mysqli_query($dbc,"SELECT * FROM reservation");
		...
		total_payment, status, reference_no, down_payment, customer_id FROM reservation LIMIT $start, $display");
	...
	switch ($_POST['choice']) {
		...
		Name, reservation_id, arrival_date, arrival_time, departure_date, departure_time, no_of_persons, days_of_rent,
		total_payment, status, reference_no, down_payment, customer_id FROM reservation WHERE CONCAT(last_name, ' ', first_name) LIKE '%$search%'");
		...
		$result2 = mysqli_query($dbc,"SELECT CONCAT(last_name, ' ',first_name,' ',middle_name) AS
		Name, reservation_id, arrival_date, arrival_time, departure_date, departure_time, no_of_persons, days_of_rent,
		total_payment, status, reference_no, down_payment, customer_id FROM reservation WHERE first_name LIKE '%$search%'");
		$num3 = @mysqli_num_rows($result2);
		if($num3 >= 1) {
		table($result2,$num3);
    	}
    	...
    	$result2 = mysqli_query($dbc,"SELECT CONCAT(last_name, ' ',first_name,' ',middle_name) AS
    	Name, reservation_id, arrival_date, arrival_time, departure_date, departure_time, no_of_persons, days_of_rent,
		total_payment, status, reference_no, down_payment, customer_id FROM reservation WHERE last_name LIKE '%$search%'");
		$num3 = @mysqli_num_rows($result2);
		if($num3 >= 1) {
		table($result2,$num3);
    	}
...
function table($rows, $nums) {
	...
	print '<div table-responsive">
				<h2>Customers Reservation</h2>';
		if($nums > 1) {
			print '<p class="para1">There are currently <span class="total">' . $nums . '</span> reserved customers..</p>';
		} else {
			print '<p class="para1">There are currently <span class="total">' . $nums . '</span> reserved customer..</p>';
		}
		...
		if ($_SESSION['user_type'] == "Administrator") {
				...
				print				'<td><b>View</b></td>';
				print				'<td><b>Archive</b></td>';
				print				'<td><b>Approve reservation</b></td>';
				print				'<td><b>Delete reservation</b></td>';
			...
			while($row = @mysqli_fetch_array($rows, MYSQLI_NUM)){
				...
				print	   		    '<td width="50"><a style="color: '. $ancolor .';" class="action" href="delete_reservation.php?reservation_id=' . $row[1] .'"><i class="fa fa-trash" style="color: #FFF"></i>&nbspDelete</a></td>';
				print				'<td width="100">' . $row[9] .'</td>';
				print				'<td width="100" align="right">' . $row[10] .'</td>';
				print				'<td width="240">' . $row[0] .'</td>';
...
if($pages > 1) {	
	...
	if($current_page != 1 ) {
			...
			print '<div class="col-md-5">';
			print '</div>';	
			print '<li><a href="reservation.php?s=' . ($start - $display) . '&p=' . $pages . '" ><i class="glyphicon glyphicon-backward"></i></a></li>';
			print '</div>';
			print '</div>';
			print '</div>';
	...
	if($current_page != $pages) {
			...
			print '<div class="col-md-5">';
			print '</div>';	
			print '<li><a href="reservation.php?s=' . ($start + $display) . '&p=' . ' '.$pages . '" ><i class="glyphicon glyphicon-forward"></i></a></li>';
			print '</div>';
			print '</div>';
			print '</div>';
	}
