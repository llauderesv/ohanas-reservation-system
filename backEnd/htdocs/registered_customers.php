...
if($_SERVER['REQUEST_METHOD']=="POST") {
	...
	if(!empty($_POST['searchs'])) {
		...
		$problem = TRUE;
		$query = mysqli_query($dbc,"SELECT * FROM customer");
		$num = mysqli_num_rows($query);
		$r = mysqli_query($dbc,"SELECT customer_id, CONCAT(last_name, ' ',first_name,' ',middle_name) AS Name, gender, cellphone_no, address, email_address, username FROM customer LIMIT $start, $display");
	...
	switch ($_POST['choice']) {
		...
		$result2 = mysqli_query($dbc,"SELECT customer_id, CONCAT(last_name, ' ',first_name,' ',middle_name) AS Name, gender, cellphone_no, address, email_address, username FROM customer WHERE CONCAT(last_name, ' ', first_name) LIKE '%$search%'");
		...
		table($result2,$num3);
			...
			break;
		case 'First Name':
		$result2 = mysqli_query($dbc,"SELECT customer_id, CONCAT(last_name, ' ',first_name,' ',middle_name) AS Name, gender, cellphone_no, address, email_address, username FROM customer WHERE first_name LIKE '%$search%'");
		$num3 = @mysqli_num_rows($result2);
		if($num3 >= 1) {
		table($result2,$num3);
    	}
    		break;
    	case 'Last Name':
    	$result2 = mysqli_query($dbc,"SELECT customer_id, CONCAT(last_name, ' ',first_name,' ',middle_name) AS Name, gender, cellphone_no, address, email_address, username FROM customer WHERE last_name LIKE '%$search%'");
		$num3 = @mysqli_num_rows($result2);
		if($num3 >= 1) {
		table($result2,$num3);
    	}
    		break;
		default:
		$result2 = mysqli_query($dbc,"SELECT customer_id, CONCAT(last_name, ' ',first_name,' ',middle_name) AS Name, gender, cellphone_no, address, email_address, username FROM customer LIMIT $start, $display");
		$num3 = @mysqli_num_rows($result2);
		table($result2,$num3);
		break;
	}
...
if($pages > 1) {	
	...
	if($current_page != 1 ) {
			...
			print '<div class="col-md-5">';
			print '</div>';	
			print '<li><a href="registered_customers.php?s=' . ($start - $display) . '&p=' . $pages . '" ><i class="glyphicon glyphicon-backward"></i></a></li>';
			print '</div>';
			print '</div>';
			print '</div>';
	...
	if($current_page != $pages) {
			...
			print '<div class="col-md-5">';
			print '</div>';	
			print '<li><a href="registered_customers.php?s=' . ($start + $display) . '&p=' . ' '.$pages . '" ><i class="glyphicon glyphicon-forward"></i></a></li>';
			print '</div>';
			print '</div>';
			print '</div>';
	}
